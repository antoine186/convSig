...
log-likelihood function as a loss function, is the ReLU transformation. The parameters
whose estimation are sought are a Feature matrix (contains the weights in the 
mutational filters for their corresponding mutational processes), a P matrix (contains
the mutational rate of each mutational process in each sample), and a mat matrix
(contains the probabilities for all mutation types; Take T mutating to C as an 
instance of a mutation type). <br />
...
a parameter indicating the number of mutational processes to extract from the input data.
